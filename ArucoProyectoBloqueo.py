...
class VentanaLaser:
    ...
    # Tamaño
        ...
        ttk.Button(control_row1, text="Grabar posición", command=self.guardar_posicion_actual, width=15).pack(side=tk.LEFT, padx=5)
        ttk.Button(control_row1, text="Ir a posición guardada", command=self.ir_a_posicion_guardada, width=18).pack(side=tk.LEFT, padx=5)
...
#           send_cmd(self.ser_laser, "G0 X0 Y0")  Ir al origen
    ...
    def iniciar_grabado(self):
        ...
        try:
            ...
            move_back_to_machine_origin(self.ser_laser)
    ...
    def guardar_posicion_actual(self):
        """Guarda la posición actual del láser"""
        if not self.ser_laser or not getattr(self.ser_laser, "is_open", False):
            self.log_laser("Conecta al láser primero")
            return
        self.ser_laser.write(b"?\n")
        time.sleep(0.2)
        respuesta = self.ser_laser.read_all().decode('utf-8', errors='ignore')
        m = re.search(r"MPos:([\d\.\-]+),([\d\.\-]+)", respuesta)
        if m:
            self.posicion_guardada = (float(m.group(1)), float(m.group(2)))
            self.log_laser(f"Posición guardada: X={m.group(1)}, Y={m.group(2)}")
        else:
            self.log_laser("No se pudo leer la posición actual")

    def ir_a_posicion_guardada(self):
        """Mueve el láser a la posición guardada y regresa a la posición inicial"""
        if not hasattr(self, 'posicion_guardada'):
            self.log_laser("No hay posición guardada")
            return
        if not self.ser_laser or not getattr(self.ser_laser, "is_open", False):
            self.log_laser("Conecta al láser primero")
            return
        x, y = self.posicion_guardada
        send_cmd(self.ser_laser, f"G0 X{x} Y{y}")
        self.log_laser(f"Láser movido a posición guardada: X={x}, Y={y}")
        move_back_to_machine_origin(self.ser_laser)
        self.log_laser("Láser regresó a la posición inicial")
...
class VentanaRobot:
    ...
    def crear_interfaz_robot(self):
        ...
        # Comandos personalizados por teclado
        custom_frame = ttk.LabelFrame(main_frame, text="Comando Personalizado", padding="5")
        custom_frame.pack(fill=tk.X, pady=(0, 10))
        
        ttk.Label(custom_frame, text="Escriba su comando:").pack(anchor=tk.W, pady=2)
        
        # Frame para entrada y botón en la misma línea
        entrada_frame = ttk.Frame(custom_frame)
        entrada_frame.pack(fill=tk.X, pady=2)
        
        self.entrada_comando = tk.Entry(entrada_frame, font=('Consolas', 10))
        self.entrada_comando.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
        
        # Vincular Enter para enviar comando
        self.entrada_comando.bind('<Return>', lambda event: self.enviar_comando_personalizado())
        
        ttk.Button(entrada_frame, text="Enviar", command=self.enviar_comando_personalizado, width=10).pack(side=tk.RIGHT)
        
        # Área de comandos rápidos adicionales
        quick_cmd_frame = ttk.Frame(custom_frame)
        quick_cmd_frame.pack(fill=tk.X, pady=2)
        ...
        for i, (text, cmd) in enumerate(quick_commands):
            btn = ttk.Button(quick_cmd_frame, text=text, width=10,
        ...
        # Frame para el log y scrollbar
        text_frame = ttk.Frame(log_frame)
        text_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 5))
        
        self.robot_log = tk.Text(text_frame, height=12, state=tk.DISABLED)
        scrollbar = ttk.Scrollbar(text_frame, orient=tk.VERTICAL, command=self.robot_log.yview)
        ...
        ttk.Button(log_frame, text="Limpiar Log", command=self.limpiar_log).pack(pady=2)
            ...
            try:
                ...
                if recibir:
                    ...
                    if "CMG" in recibido or "CHG" in recibido:
                        self.log_robot(f"Mensaje recibido: {recibido}")
                time.sleep(1)
            except Exception as e:
                self.log_robot(f"❌ Error en escucha: {e}")
                break
    
    def enviar_comando_personalizado(self):
        """Envía un comando personalizado escrito por el usuario"""
        ...
        if comando:
            ...
            try:
                # Enviar comando
                cmd_enviar = comando.upper() + '\r'
                self.ser_robot.write(cmd_enviar.encode())
                self.log_robot(f"📤 Enviado: {comando}")
                
                # Leer respuesta
                time.sleep(0.5)
                respuesta = self.ser_robot.read_all()
                if respuesta:
                    respuesta_str = respuesta.decode('utf-8', errors='ignore').strip()
                    self.log_robot(f"📥 Respuesta: {respuesta_str}")
                
                # Limpiar el campo después de enviar
                self.entrada_comando.delete(0, tk.END)
                
            except Exception as e:
                self.log_robot(f"❌ Error enviando comando: {e}")
        else:
    ...
    def enviar_comando_rapido(self, comando):
        ...
        self.enviar_comando_personalizado()
    
    def limpiar_log(self):
        """Limpia el log del robot"""
        self.robot_log.config(state=tk.NORMAL)
        self.robot_log.delete("1.0", tk.END)
        self.robot_log.config(state=tk.DISABLED)
        self.log_robot("🧹 Log limpiado")
...
class VentanaScorbotOriginal:
    ...
    def crear_interfaz_scorbot(self):
        # Configuración de estilo AISLADA (no sobreescribir estilos globales)
        style = ttk.Style()
        style.configure('Scorbot.TButton', font=('Arial', 9), padding=5)
        style.configure('Scorbot.TLabel', font=('Arial', 9))
        style.configure('Scorbot.TLabelframe.Label', font=('Arial', 9, 'bold'))
        ...
        ttk.Button(frame_estado, text="Conectar", command=self.click_conectar, style='Scorbot.TButton').pack(side=tk.LEFT, padx=5)
        ttk.Button(frame_estado, text="Desconectar", command=self.click_desconectar, style='Scorbot.TButton').pack(side=tk.LEFT, padx=5)
        ...
        cmd_frame = ttk.LabelFrame(main_frame, text="Comandos Rápidos", padding=10, style='Scorbot.TLabel')
        ...
        for text, cmd in buttons:
            ttk.Button(cmd_frame, text=text, command=cmd, style='Scorbot.TButton').pack(fill=tk.X, pady=2)

        # Panel de comunicación
        comm_frame = ttk.LabelFrame(main_frame, text="Comunicación", padding=10, style='Scorbot.TLabel')
        comm_frame.grid(row=0, column=1, padx=5, pady=5, sticky=tk.NSEW)

        # Sección de comando personalizado mejorada
        cmd_custom_frame = ttk.LabelFrame(comm_frame, text="Comando Personalizado", padding=5, style='Scorbot.TLabel')
        cmd_custom_frame.pack(fill=tk.X, pady=(0, 10))
        
        ttk.Label(cmd_custom_frame, text="Escriba su comando:", style='Scorbot.TLabel').pack(anchor=tk.W, pady=2)
        
        # Frame para entrada y botón en la misma línea
        entrada_frame = ttk.Frame(cmd_custom_frame)
        entrada_frame.pack(fill=tk.X, pady=2)
        
        self.TextEnviar = tk.Entry(entrada_frame, font=('Consolas', 10))
        self.TextEnviar.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
        
        # Vincular Enter para enviar comando
        self.TextEnviar.bind('<Return>', lambda event: self.click_enviar())
        
        ttk.Button(entrada_frame, text="Enviar", command=self.click_enviar, width=10, style='Scorbot.TButton').pack(side=tk.RIGHT)
        
        # Área de comandos rápidos adicionales
        quick_cmd_frame = ttk.Frame(cmd_custom_frame)
        quick_cmd_frame.pack(fill=tk.X, pady=2)
        ...
        for i, (text, cmd) in enumerate(quick_commands):
            btn = ttk.Button(quick_cmd_frame, text=text, width=8,
                           command=lambda c=cmd: self.enviar_comando_rapido(c), style='Scorbot.TButton')
            btn.grid(row=0, column=i, padx=2, pady=2)

        # Área de recepción
        ttk.Label(comm_frame, text="Respuesta del robot:", style='Scorbot.TLabel').pack(anchor=tk.W, pady=(10, 2))
        self.TextRecibidos = tk.Text(comm_frame, height=12, width=50, font=('Consolas', 9))
        self.TextRecibidos.pack(fill=tk.BOTH, expand=True, pady=2)

        # Botones de control
        btn_frame = ttk.Frame(comm_frame)
        btn_frame.pack(fill=tk.X, pady=5)
        
        ttk.Button(btn_frame, text="Limpiar Log", command=self.limpiar_log, style='Scorbot.TButton').pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(btn_frame, text="Guardar Log", command=self.click_guardar, style='Scorbot.TButton').pack(side=tk.LEFT)
        
        # Botón Cerrar
        ttk.Button(self.ventana, text="Cerrar", command=self.cerrar_ventana, style='Scorbot.TButton').pack(pady=10)
    ...
    def click_enviar(self):
        custom_cmd = self.TextEnviar.get().strip()
        if custom_cmd:
            self.send_scorbot_command(custom_cmd, "Comando personalizado enviado")
            # Limpiar el campo después de enviar
            self.TextEnviar.delete(0, tk.END)
        else:
            messagebox.showwarning("Advertencia", "Escriba un comando primero")
    ...
    def enviar_comando_rapido(self, comando):
        """Envía un comando rápido predefinido"""
        self.TextEnviar.delete(0, tk.END)
        self.TextEnviar.insert(0, comando)
        self.send_scorbot_command(comando, f"Comando rápido: {comando}")
    
    def limpiar_log(self):
        """Limpia el área de respuestas del robot"""
        self.TextRecibidos.delete("1.0", tk.END)
    ...
    def insertar_comando_externo(self, comando):
        """Permite insertar un comando desde fuente externa (ej: comun.py)"""
        if self.TextEnviar:
            # Limpiar el campo de texto actual
            self.TextEnviar.delete(0, tk.END)
            # Insertar el nuevo comando
            self.TextEnviar.insert(0, comando)
            # Hacer focus al campo para que sea visible al usuario
            self.TextEnviar.focus()
            print(f"Comando insertado desde fuente externa: {comando}")
            
    def cerrar_ventana(self):
        """Cerrar ventana del scorbot original"""
        self.click_desconectar()
        
        # Limpiar referencias de comun.py
        if comun is not None:
            try:
                comun.TextEnviar = None
                comun.TextRecibidos = None
                comun.TextoEstado = None
                comun.comboBox1 = None
                comun.SerialPort1 = None
                print("Referencias de comun.py limpiadas")
            except:
...
class Aplicacion:
    ...
    # Selector de índice de cámara
        frame_cam_select = ttk.Frame(frame_video)
        frame_cam_select.pack(side=tk.TOP, anchor="center", pady=5)
        
        ttk.Label(frame_cam_select, text="Índice de cámara:").pack(side=tk.LEFT, padx=5)
        self.cam_index_var = tk.IntVar(value=0)
        self.spinbox_cam = tk.Spinbox(frame_cam_select, from_=0, to=10, width=5, 
                                     textvariable=self.cam_index_var)
        self.spinbox_cam.pack(side=tk.LEFT, padx=5)
    ...
    def abrir_ventanas_robot_originales(self):
        ...
        try:
            ...
            if self.ventana_scorbot_original is None or not self.ventana_scorbot_original.ventana.winfo_exists():
                ...
                # Integrar con comun.py: hacer que las variables de comun apunten a esta ventana
                if comun is not None:
                    comun.TextEnviar = self.ventana_scorbot_original.TextEnviar
                    comun.TextRecibidos = self.ventana_scorbot_original.TextRecibidos
                    comun.TextoEstado = self.ventana_scorbot_original.TextoEstado
                    comun.comboBox1 = self.ventana_scorbot_original.comboBox1
                    comun.SerialPort1 = self.ventana_scorbot_original.SerialPort1
                    print("Integración con comun.py establecida")
    ...
    def iniciar_camara(self):
        # Obtener el índice seleccionado por el usuario
        indice_usuario = self.cam_index_var.get()
        
        # Probar primero con el índice seleccionado
        camara_abierta = False
        indice_usado = None
        
        # Lista de índices a probar: primero el seleccionado, luego otros como respaldo
        indices_a_probar = [indice_usuario]
        for idx in range(5):  # Ampliar rango a 0-4
            if idx != indice_usuario:
                indices_a_probar.append(idx)
        ...
        for idx in indices_a_probar:
            ...
            if self.camara.isOpened():
                camara_abierta = True
                indice_usado = idx
                print(f"Cámara abierta exitosamente con índice {idx}")
                break
            else:
                self.camara.release()
        
        if not camara_abierta:
            error_msg = f"No se pudo abrir ninguna cámara (probados índices: {indices_a_probar})"
            print(error_msg)
            self.lbl_video.config(text=error_msg)
            return

        # Mostrar qué índice se está usando
        if indice_usado == indice_usuario:
            status_msg = f"Usando cámara índice {indice_usado} (seleccionado)"
        else:
            status_msg = f"Usando cámara índice {indice_usado} (respaldo - el índice {indice_usuario} no funcionó)"
        print(status_msg)
    ...
    def actualizar_etiquetas(self, img_tk):
